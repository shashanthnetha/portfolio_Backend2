...
const paintCinematic: ScreenPainter = (context, width, height, time) => {
  ...
  const phase = time;
  const counting = phase < 5;
  const label = Math.max(1, 5 - Math.floor(phase));
  ...
  if (counting) {
    ...
    const flash = Math.max(0, 1 - (phase - 5) / 0.18);
    if (flash > 0) {
      context.fillStyle = `rgba(250,250,252,${(flash * 0.85).toFixed(3)})`;
